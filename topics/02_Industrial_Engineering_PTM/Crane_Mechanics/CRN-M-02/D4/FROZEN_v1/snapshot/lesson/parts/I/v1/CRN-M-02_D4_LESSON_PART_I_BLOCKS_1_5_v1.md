...
## Reader Navigator — как читать D4, не запоминая 45 блоков

Эта часть задаёт **операционную систему** всего D4. Не пытайся запомнить названия всех standards, models и guards. Держи в рабочей памяти маршрут:
...
→ VERIFICATION + VALIDATION/CROSS-CHECK
→ SENSITIVITY / UNCERTAINTY
→ BOUNDED CONCLUSION
→ AUTHORITY HANDOFF
```

В следующих частях этот маршрут просто разворачивается по слоям:

```text
I   — зачем и каким методом отвечаем
II  — что именно моделируем и в каком состоянии
III — какая fidelity действительно нужна
IV  — где начинается source-bound formal proof
V   — как связать global и local component models
VI  — чем проверяем implementation и adequacy
VII — что способно перевернуть решение
VIII— как duty / inspection / monitoring входят как разные evidence channels
IX  — как собрать review и остановиться в правильном месте
```

### Что не надо зубрить

Не нужно держать в памяти номер каждого ISO или каждую формулировку `≠`. Нужно уметь восстановить различие своими словами: **какой вопрос, какой метод, почему применим, откуда входы, чем проверен, насколько устойчив вывод и кто имеет право действовать**.

### К концу Части I ты должен уметь

- превратить расплывчатое «проверить барабан» в конкретный question class;
- отличить requirement, selection/proof, inspection и monitoring role;
- заполнить method passport и applicability record до расчёта;
- построить claim ladder без скачка от результата модели к equipment decision;
- оставить audit trail, который другой инженер сможет воспроизвести.
...
## Reader Checkpoint I — закрой текст и восстанови маршрут

1. Почему один и тот же объект может требовать разных methods?
2. Какие четыре поля ты запишешь до выбора расчётной модели?
3. Что должно появиться между `MODEL RESULT` и `EQUIPMENT DECISION`?
4. Какой признак говорит, что applicability ещё не доказана?
5. Что обязан увидеть следующий инженер в audit trail, кроме финального успешного расчёта?
