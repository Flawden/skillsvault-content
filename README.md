...
Status: `COMPLETE`. Ten Practice cases A–J now have non-exclusive Russian-first causal-diagnostic reasoning paths. Reasoning preserves competing-hypothesis independence, common-mode premise checks, condition-matched predictions, observability-aware negative evidence, circular-evidence rejection, state/transient/history separation, temporal cause/consequence discipline and parallel source-authority lanes. Twelve cross-case failure families are passed forward as Expected Result candidates; no automatic BF status is assigned here.
